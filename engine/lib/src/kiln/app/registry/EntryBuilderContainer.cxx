...
auto EntryBuilderContainer::bubble_up_dependencies_of(
    const uint64_t             hash,
...
auto EntryBuilderContainer::try_index_of_builder(const uint64_t hash)
...
auto EntryBuilderContainer::collect_dependencies_of(
    const uint64_t              hash,
...
) -> void
{
    ...
    {
        for (const uint64_t dependency_hash : m_dependency_entry_hashes[builder_index])
        {
