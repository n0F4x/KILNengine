...
module kiln.exec.data_structures.SignalTree;
...
auto SignalTree::number_of_levels() const noexcept -> uint32_t
{
    return static_cast<uint32_t>(std::bit_width(m_nodes.size()));
}

auto SignalTree::number_of_leaves() const noexcept -> uint32_t
{
    return 0x1u << (number_of_levels() - 1);
}

auto SignalTree::number_of_branches() const noexcept -> uint32_t
{
    return number_of_nodes() - number_of_leaves();
}

auto SignalTree::number_of_nodes() const noexcept -> uint32_t
{
...
) -> std::optional<LeafIndex>
{
    ...
    const TraversalBias traversal_bias{ strategy(node_index) };
    ...
    NodeIndex                visited_child_index{ traversal_bias == TraversalBias::eLeft
                                                      ? left_child_index
                                                      : right_child_index };
...
auto SignalTree::try_unset_one_at(const NodeIndex node_index, const NodeIndex target_index)
    -> bool
{
    Counter::value_type counter{ m_nodes[node_index].counter.load() };
    do
...
auto SignalTree::try_unset_leaf(const NodeIndex node_index) -> std::optional<LeafIndex>
{
...
auto SignalTree::is_leaf_index(const NodeIndex node_index) const noexcept -> bool
{
    return node_index >= number_of_branches();
}
